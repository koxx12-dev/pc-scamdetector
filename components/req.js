...
/*
* stolen from powercord code (i like the powercord request api)
*/
...
class reqst {
  constructor(method, uri) {
    this.opts = {
      ...
      query: {},
      headers: {
        "User-Agent": `Powercord`,
      },
    };
  }

  _objectify(key, value) {
    return key instanceof Object ? key : { [key]: value };
  }
      ...
      const req = request(options, (res) => {
        ...
        res.once("end", () => {
          ...
          if (result.ok) {
            ...
            reject(
              new HTTPError(`${res.statusCode} ${res.statusMessage}`, result)
            );
          }
        });
      });
